/*
 * SimpleTimerDelegate.cpp
 *
 * SimpleTimerDelegate - A timer library for Arduino.
...
SimpleTimerDelegate::SimpleTimerDelegate() {
...
int SimpleTimerDelegate::setTimer(long d, timer_delegate f, int n) {
...
int SimpleTimerDelegate::setTimer(long d, timer_delegate f, int n, int param) {
    if (num_timers >= MAX_TIMERS) {
        return -1;
    }
...
int SimpleTimerDelegate::setInterval(long d, timer_delegate f) {
    return setTimer(d, f, RUN_FOREVER);
}


int SimpleTimerDelegate::setTimeout(long d, timer_delegate f) {
    return setTimer(d, f, RUN_ONCE);
}


void SimpleTimerDelegate::deleteTimer(int numTimer) {
...
boolean SimpleTimerDelegate::isEnabled(int numTimer) {
    if (numTimer >= MAX_TIMERS) {
        return false;
    }
...
void SimpleTimerDelegate::enable(int numTimer) {
    if (numTimer >= MAX_TIMERS) {
        return;
    }
...
void SimpleTimerDelegate::disable(int numTimer) {
    if (numTimer >= MAX_TIMERS) {
        return;
    }
...
void SimpleTimerDelegate::toggle(int numTimer) {
    if (numTimer >= MAX_TIMERS) {
        return;
    }
...
int SimpleTimerDelegate::getAvailableSpot() {
	for (int i = 0; i < MAX_TIMERS; i++) {
...
int SimpleTimerDelegate::getNumTimers() {
    int numTimers = 0;
    for (int i = 0; i < MAX_TIMERS; i++) {
